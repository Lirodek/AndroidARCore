...
//                    "}";
    ...
    public void draw() {
        ...
        int lighting = GLES20.glGetUniformLocation(mProgram, "uLighting"); // uColorCorrection
        ...
        GLES20.glUniform4f(colorCorrection, viewLightDirection[0], mColorCorrection[1], mColorCorrection[2], mColorCorrection[3]);
    ...
    void setColorCorrection(float[] colorCorrection){
        for(int i=0; i<colorCorrection.length; i++)
            mColorCorrection[i] = colorCorrection[i];
    }

    float[] myMinPoint, myMaxPoint;
    public float[][] getMinMaxPoint(){
        calcMyMinMax();
        ...
        Matrix.multiplyMV(minPoint, 0, mModelMatrix, 0, new float[]{myMinPoint[0], myMinPoint[1], myMinPoint[2],1f}, 0);
        float[] maxPoint = new float[4];
        Matrix.multiplyMV(maxPoint, 0, mModelMatrix, 0, new float[]{myMaxPoint[0], myMaxPoint[1], myMaxPoint[2],1f}, 0);

        float[] res0 = new float[3];
        for(int i=0; i<3; i++){
            res0[i] = Math.min(minPoint[i], maxPoint[i]);
        }
        float[] res1 = new float[3];
        for(int i=0; i<3; i++){
            res1[i] = Math.max(minPoint[i], maxPoint[i]);
        }
    ...
    public void calcMyMinMax(){
        ...
        /*
        getNumVertices() 점의 개수수
       점1, 점2, 점3, 점4
        vertices(점1,x, 점1.y, 점2.x . . .)
         */
        for(int i=1;i<mObj.getNumVertices(); i++){
           for(int j=0;j<3;j++){
               myMinPoint[j] = Math.min(myMinPoint[j], vertices[i*3+j]);
               myMaxPoint[j] = Math.max(myMaxPoint[j], vertices[i*3+j]);
           }
        }
